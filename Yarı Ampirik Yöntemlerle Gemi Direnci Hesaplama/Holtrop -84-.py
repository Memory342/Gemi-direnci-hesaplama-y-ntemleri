# -*- coding: utf-8 -*-
"""
Created on Tue Dec  5 23:59:06 2023
...
#Required and optional input parameters for Holtrop-Mennen's Method (1984)
...
#FORM FACTOR
k = float(input('Enter the form factor, 1+k: ')) 
if k == 0:
    k = 0.93 + (0.487118*c14) * ((beam/length_wl)**1.06806) * ((t/length_wl)**0.46106) * ((length_wl/l_r)**0.121563) * (((length_wl**3)/disp_v)**0.36486) * ((1-c_p)**-0.604247)
else:
    k = k
...
m4 = 0.4 * c15 * (np.e**(-0.034*(froudes**-3.29)))
...
c17 = (6919.3 * (c_m**-1.3346)) * ((disp_v/length_wl**3)**2.00977) * (((length_wl/beam)-2.0)**1.40692)

m3 = (-7.2035*((beam/length_wl)**0.326869)) * ((t/beam)**0.605375)


# Initialize empty r_w array
r_w = np.empty_like(froudes, dtype=float)

# Calculate m4 for all froudes values
m4 = 0.4 * -1.69385 * (np.e**(-0.034*(froudes**-3.29)))
m4_4 = 0.4 * -1.69385 * (np.e**(-0.034*(0.4**-3.29)))
m4_55 = 0.4 * -1.69385 * (np.e**(-0.034*(0.55**-3.29)))

# Conditions for different ranges
condition1 = froudes <= 0.4
condition2 = froudes > 0.55
condition3 = (0.4 < froudes) & (froudes <= 0.55)
r_wa4 = c1 * c2 * c5 * rho * g * disp_v * np.exp((m1 * (0.4**d)) + (m4_4 * np.cos(lamda * (0.4**-2)))) 
r_wb55 = c17 * c2 * c5 * rho * g * disp_v * np.exp((m3 * (0.55**d)) + (m4_55 * np.cos(lamda * (0.55**-2))))

# Calculate r_w for each condition using vectorized operations
r_w[condition1] = c1 * c2 * c5 * rho * g * disp_v * np.exp((m1 * (froudes[condition1]**d)) + (m4[condition1] * np.cos(lamda * (froudes[condition1]**-2))))
r_w[condition2] = c17 * c2 * c5 * rho * g * disp_v * np.exp((m3 * (froudes[condition2]**d)) + (m4[condition2] * np.cos(lamda * (froudes[condition2]**-2))))
r_w[condition3] = r_wa4 + ((20 * froudes[condition3] - 8) / 3) * (r_wb55 - r_wa4 )
...
# Plotting r_t
plt.plot(froudes, r_t, label='Total Resistance', color='orange', marker = 'o')
...
plt.title('Total Resistance vs Froudes Number (Holtrop-Mennen 1984)')
...
# Plotting p_e
plt.plot(froudes, p_e, label='Effective Power (W)', color='orange', marker = 'o')
plt.plot(froudes, real_p_e, label='Experimental Effective Power (W)' , color = 'blue', marker = '*')

# Adding labels and title
plt.xlabel('Froudes Number')
plt.ylabel('Effective Power [W]')
plt.title('Effective Power vs Froudes Number (Holtrop-Mennen 1984)')
plt.grid(color = 'gray', linestyle = '--', linewidth = 0.5)
plt.legend()
...
plt.title('All Resistance vs Froudes Number (Holtrop-Mennen 1984)')
plt.grid(color = 'gray', linestyle = '--', linewidth = 0.5)
plt.legend()
...
plt.plot(froudes, real_p_e, label='Effective Power (W)', color='green', marker = 'o')

# Adding labels and title
plt.xlabel('Froudes Numbers')
plt.ylabel('Effective Power [W]')
plt.title('Effective Power vs Froudes Number')
plt.grid(color = 'gray', linestyle = '--', linewidth = 0.5)
plt.legend()
