# -*- coding: utf-8 -*-
"""
Created on Fri Dec  1 01:02:24 2023
...
#Required and optional input parameters for Holtrop-Mennen's Method (1978)
...
#THE WAVE RESISTANCE
half_alpha = (125.67*(beam/length_wl)) - (162.25*(c_p**2)) + (234.32*(c_p**3)) + (0.155087*(lcb + ((6.8*(t_a - t_f)) / t) )**3)
c3 = (0.56*(abt**1.5)) / ( beam*t*((0.56*np.sqrt(abt)) + t_f - hb - (0.25*np.sqrt(abt))) )
m2 = -1.69385 * (c_p**2) * np.e**(-0.1/(froudes**2))
m1 = (0.0140407 * (length_wl/t)) - (1.75254 * ((disp_v**(1/3))/length_wl)) - (4.79323 * (beam/length_wl)) - 8.07981*c_p + 13.8673*(c_p**2) - 6.984388*(c_p**3)
c2 = np.e**(-1.89*np.sqrt(c3))
c1 = (2223105*((beam/length_wl)**3.78613)) * ((t/beam)**1.07961) * ((90 - half_alpha)**(-1.37565))
d = -0.9
lamda = (1.446*c_p) - (0.03*(length_wl/beam))
r_w = disp * c1 * c2 * np.e**( (m1*(froudes**d)) + (m2*np.cos(lamda * (froudes**-2))) )

#############################################################################################################################################

#THE RESİSTANCE OF BULBOUS BOW
p_b = (0.56 * np.sqrt(abt)) / (t_f - 1.5*hb)
c = 0.11 * np.e**(-3 * (p_b**-2)) * (abt**1.5) * rho * g
i = t_f - hb - (0.25*np.sqrt(abt))
froudes_i = speeds / np.sqrt((g*i) + 0.15*(speeds**2))
r_b = (c * (froudes_i**3)) / (1 + (froudes_i**2))

#############################################################################################################################################

#THE CORRELATION ALLOWANCE
length_m = 7.5 #♣Typical model length
length_s = length_wl #Length on the waterline

if (t_f/length_s) <= 0.04:
    c4 = t_f/length_s
elif (t_f/length_s) > 0.04:
    c4 = 0.04

c_a = 0.006*((length_s+100)**-0.16)  - 0.00205
r_a = c_a * 0.5 * rho * (speeds**2) * s 
...
# Plotting r_t
plt.plot(froudes, r_t, label='Total Resistance', color='green', marker = 'o')
...
plt.title('Total Resistance vs Froudes Number (Holtrop-Mennen 1978)')
...
# Plotting p_e
plt.plot(froudes, p_e, label='Effective Power (W)', color='green', marker = 'o')
plt.plot(froudes, real_p_e, label='Experimental Effective Power (W)' , color = 'blue', marker = '*')

# Adding labels and title
plt.xlabel('Froudes Number')
plt.ylabel('Effective Power [W]')
plt.title('Effective Power vs Froudes Number (Holtrop-Mennen 1978)')
plt.grid(color = 'gray', linestyle = '--', linewidth = 0.5)
plt.legend()
...
plt.title('All Resistance vs Froudes Number (Holtrop-Mennen 1978)')
plt.grid(color = 'gray', linestyle = '--', linewidth = 0.5)
plt.legend()
